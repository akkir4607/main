...
const TextPressure = ({
  ...
  const setSize = useCallback(() => {
    ...
    let newFontSize = containerW / (chars.length * 0.55);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (titleRef.current) {
        ...
        const maxDist = Math.max(titleRect.width, window.innerWidth) / 2;
  ...
  const styleElement = useMemo(() => {
    return (
      <style>{`
        ...
        .stroke-${uniqueId} span {
          position: relative;
          color: transparent;
          -webkit-text-stroke: 1.5px ${textColor};
        }
        .stroke-${uniqueId} span::after {
        ...
        .text-pressure-title-${uniqueId} {
          color: ${textColor};
        }
      `}</style>
    );
  }, [fontFamily, fontUrl, textColor, strokeColor, uniqueId]);

  const dynamicClassName = [
    className,
    flex ? `flex-${uniqueId}` : '',
    stroke ? `stroke-${uniqueId}` : ''
  ].filter(Boolean).join(' ');
  ...
  return (
    ...
    >
      ...
      <h1
        ref={titleRef}
        className={`text-pressure-title-${uniqueId} ${dynamicClassName}`}
        style={{
          fontFamily,
